...
var createGroups = function (db, searchText, callback) {
    //var srchText = "Error";
    ...
    var pipeline = [
        ...
        },
        {
            ...
            $project: {
                ...
                procPath: 1,
                bkgroundInfo: 1,
                "result": { $cond: { if: { $gte: [{ $indexOfCP: ["$bkgroundInfo", srchText] }, 0] }, then: { $substrCP: ["$bkgroundInfo", { $indexOfCP: ["$bkgroundInfo", srchText] }, 140] }, else: "Not Found" } }
            }
        },
        {
            "$out": "SKSNodeColl"
...
router.post('/userdetails', function (req, res, next) {
    var searchText = req.body.searchText ? req.body.searchText : "Error";    
    console.log("sks search: " + searchText); //Output: Error
    //console.log(req.query.searchText); //Output: Undefined
    //console.log(req.body); //output: { searchText: 'Error' }
    //console.log(req.params.searchText); //output: undefined
    MongoClient.connect(url, { useNewUrlParser: true }, function (err, client) {
        if (err) { console.log(err); throw err; }
        var db = client.db("mydb");
        var collection = db.collection("Users");
        //console.log("Mongo Connection - New Collection");
        createGroups(collection, searchText, function (err, result) {
