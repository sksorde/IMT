...
employeeController.list = function (req, res) {
    Employee.find({}).exec(function (err, incidents) {
        if (err) {
            console.log("Error:", err);
        }
        else {

            res.render("../views/employees/index", { employees: incidents });
        }
    });
};


employeeController.stage3 = function (req, res) {
    Employee.find({area: 'NEW'}).exec(function (err, NEW) {
        if (err) {
            console.log("Error:", err);
        }
        else {
		
            res.render("../views/employees/index", { employees: NEW });
        }
    });
};


employeeController.ply = function (req, res) {
    Employee.find({area: 'PLY'}).exec(function (err, PLY) {
        if (err) {
            console.log("Error:", err);
        }
        else {
		
            res.render("../views/employees/index", { employees: PLY });
        }
    });
};
...
employeeController.show = function (req, res) {
    Employee.findOne({ _id: req.params.id }).exec(function (err, employee) {
		...
		EmployeeDetail.find({ empid: req.params.id }).exec(function (err, employeedetail) {
		        if (err) {
        		    console.log("Error:", err);
	        	}
		res.render("../views/employees/show", { employee: employee, employeedetail: employeedetail });
		});	
...
employeeController.delete = function (req, res) {
    Employee.remove({ _id: req.params.id }, function (err) {
		...
		EmployeeDetail.remove({ empid: req.params.id }, function (err) {

			 if (err) {
        			    console.log(err);
		        }
			 console.log("Employee detail deleted!");
		});	    	
